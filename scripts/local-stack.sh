...
# The demo market is optional, so the list is built rather than interpolated
# from a fixed pair — a trailing comma would make the indexer refuse an empty
# address, which is a confusing way to learn the demo market was switched off.
INDEXED_MARKETS="$MARKET"
if [ -n "${DEMO_MARKET_ADDR:-}" ]; then
  INDEXED_MARKETS="$MARKET,$DEMO_MARKET_ADDR"
fi
...
VAULT_ADDRESS=$VAULT
POOL_ADDRESS=$POOL
# Every market, not the primary one. Since GHO-43 the indexer keys round events
# by (market, round id) and watches a list — before that it could cover exactly
# one, and a position in the demo market read back as no position at all.
MARKET_ADDRESSES=$INDEXED_MARKETS
