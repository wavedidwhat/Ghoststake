...
log "deploying"
DEPLOY_OUT="$(forge script script/Deploy.s.sol:Deploy --rpc-url "$RPC_URL" --broadcast --slow 2>&1)"
...
ASSET_ADDRESS="$ASSET" POOL_ADDRESS="$POOL" VAULT_ADDRESS="$VAULT" MARKET_ADDRESS="$MARKET" \
  forge script script/Seed.s.sol:Seed --rpc-url "$RPC_URL" --broadcast --slow 2>&1 |
  sed -n '/=== seeded ===/,/healthFactor/p'
